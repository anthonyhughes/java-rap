...
package com.datalanguage.javarap.utils;

import com.datalanguage.javarap.model.CorreferencialPair;
import com.datalanguage.javarap.model.NP;
import com.datalanguage.javarap.model.TagWord;
...
//        System.err.println("cmd /c  "+command + " " + System.getProperty("parserOption") + dataDir + " " + inputFile);
    ...
    public static Vector resolverV1(Vector aNPList, Vector aPRPList) {
        ...
        int threshhold = 30;
        WordSalienceComparator wordSalienceComparator = new WordSalienceComparator();
        ...
        while (prpIterator.hasNext()) {
            ...
            if (!foundMatcher) {
                ...
                Arrays.sort(sortedCandidates, wordSalienceComparator);
    ...
    public static String processResult(Object np, Object referer) {
    ...
    static public String substitutionV0(DefaultMutableTreeNode root,
        ...
        while (iterator.hasNext()) {
            ...
            if (cp.getReferee() == null) {
                continue;
            }
            int refererSIdx = cp.getReferer().getSentenceIdx();
            int leng = cp.getReferer().getContent().length();
            aSentStr = sentArray[refererSIdx].toString();
            //Locate the beginning position of the referer in the possiblly altered sentence.
            int begin = findOffset(aSentStr, cp.getReferer().getOffset());
            sentArray[refererSIdx].replace(begin
                    , begin + leng
                    ,
                    "<" + cp.getReferee().getSubstitutedContent() + ">");
        }
    ...
    static public String substitution(DefaultMutableTreeNode root, Vector result,
        ...
        while (iterator.hasNext()) {
            ...
            if (cp.getReferee().toStringBrief().equals(cp.getReferer().toStringBrief())) {
                continue;
            }

            int refererSIdx = cp.getReferer().getSentenceIdx();

            String str = cp.getReferee().getSubstitutedContent();
            if (cp.getReferer().getTag().equalsIgnoreCase("PRP$")) {
                if (cp.getReferer().getNumber() == 2) {
                    str += "'";
                } else {
                    str += "'s";
                }
            ...
            try {
                if (System.getProperty("mode").equals("TagPresent")) {
                    sentArray[refererSIdx][cp.getReferer().getOffset()] = "<" + str + ">";
                } else {
                    sentArray[refererSIdx][cp.getReferer().getOffset()] = str;
                }
            } catch (Exception ex) {
    ...
    static public String parseSingleSentence(String sent) {
        String output = null;
